...
import { additionalData, leaderboardData, LeaderboardDataType } from '../../utils/leaderboardData';
import './LeaderBoard.css';

const LeaderBoard: FC = () => {
  const [isMenuOpened, setMenuOpened] = useState<boolean>(false);
  const [leaderboardUsers, setLeaderboardUsers] = useState<LeaderboardDataType[] | []>([]);
  ...
  const winButtonClass = `leaderboard__sort-variant ${isSortedBy === 'win'
    ? 'leaderboard__sort-variant_active'
    : ''}`;

  const gamesButtonClass = `leaderboard__sort-variant ${isSortedBy === 'games'
    ? 'leaderboard__sort-variant_active'
    : ''}`;

  const rankButtonClass = `leaderboard__sort-variant ${isSortedBy === 'rank'
    ? 'leaderboard__sort-variant_active'
    : ''}`;
      ...
      <div className="leaderboard__container">
        <div className="leaderboard__sort-menu">
          <button className="leaderboard__sort-button" onClick={handleOpenMenu}>
            ...
            <span className={`leaderboard__menu-icon${isMenuOpened ? ' leaderboard__menu-icon_opened' : ''}`}></span>
          </button>
          <ul className={`leaderboard__dropdown${isMenuOpened ? ' leaderboard__dropdown_opened' : ''}`}>
            <li className="leaderboard__dropdown-item">
              <button className={winButtonClass}
                type="button" onClick={() => handleSort('win')}>Высокий процент побед</button>
            </li>
            <li className="leaderboard__dropdown-item">
              <button className={gamesButtonClass}
                type="button" onClick={() => handleSort('games')}>Количество сыгранных игр</button>
            </li>
            <li className="leaderboard__dropdown-item">
              <button className={rankButtonClass}
                type="button" onClick={() => handleSort('rank')}>Общий рейтинг</button>
            </li>
          </ul>
        </div>
        ...
        <button className="leaderboard__show-more"
          onClick={showMore} disabled={isProccessing}>
          {buttonTitle}
        </button>
      </div>
    </main>
